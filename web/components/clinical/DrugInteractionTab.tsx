...
export function DrugInteractionTab() {
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold text-slate-800 mb-1 dark:text-gray-100">{t("clinical.drug.heading")}</h3>
      <p className="text-sm text-slate-500 mb-4 dark:text-gray-400">{t("clinical.drug.desc")}</p>
      ...
      {data && (
        <>
            ...
            <div className="space-y-6">
              {data.interactions.map((drug, i) => (
                <div key={i} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                  <h4 className="text-base font-semibold text-slate-800 mb-3 flex items-center gap-2 dark:text-gray-100">
                  ...
                  {drug.contraindications.length > 0 && (
                    <div className="mb-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-red-600 mb-2 dark:text-red-400">
                        {t("clinical.drug.contraindications")}
                      </p>
                      <div className="space-y-2">
                        {drug.contraindications.map((item, j) => (
                          <div key={j} className="rounded-lg border border-red-200 bg-red-50/50 p-3 dark:border-red-800 dark:bg-red-900/20">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-medium text-red-800 dark:text-red-300">{item.nutrient}</span>
                              <span className="text-xs text-red-500 dark:text-red-400">({item.nutrient_type})</span>
                            </div>
                            <p className="text-sm text-slate-700 dark:text-gray-200">{item.context}</p>
                            <EvidenceBlock
                              evidenceList={[{
                  ...
                  {drug.complements.length > 0 && (
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-wide text-blue-600 mb-2 dark:text-blue-400">
                        {t("clinical.drug.complements")}
                      </p>
                      <div className="space-y-2">
                        {drug.complements.map((item, j) => (
                          <div key={j} className="rounded-lg border border-blue-200 bg-blue-50/50 p-3 dark:border-blue-800 dark:bg-blue-900/20">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-medium text-blue-800 dark:text-blue-300">{item.nutrient}</span>
                              <span className="text-xs text-blue-500 dark:text-blue-400">({item.nutrient_type})</span>
                            </div>
                            <p className="text-sm text-slate-700 dark:text-gray-200">{item.context}</p>
                            <EvidenceBlock
                              evidenceList={[{
          ...
          {data.disclaimer && (
            <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
              {data.disclaimer}
            </p>
          )}
        </>
